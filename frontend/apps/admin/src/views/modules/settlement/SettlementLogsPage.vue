...
function exportCsv() {
  ...
  const rows = filteredLogs.value.map((x) => [formatTs(x.created_at), x.merchant_id, x.order_no, x.change_type, String(x.amount), String(x.balance_before), String(x.balance_after), x.reason || ''])
