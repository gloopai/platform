  ...
  <div class="space-y-6">
    ...
    <div class="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-sm">
      ...
      <p class="mt-2 text-sm leading-relaxed text-slate-600">
        这里配置右上角头像下拉中、<strong>退出登录之上</strong>的快捷入口；每项为一级页面，需填写前端路由 path。
      </p>
    </div>
          ...
          <div v-else class="max-h-[min(70vh,520px)] divide-y divide-slate-100 overflow-y-auto">
            ...
            >
              <div class="min-w-0 flex-1">
                <div class="font-medium text-slate-900">{{ m.label }}</div>
      ...
      <div class="lg:col-span-8 space-y-4">
        <div class="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-sm">
          ...
          <div class="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            <label class="grid gap-1 text-xs font-medium text-slate-600">
              图标 key
              <input v-model.trim="form.icon" type="text" class="rounded-lg border border-slate-200 px-3 py-2 text-sm" placeholder="与侧栏一致，如 cog" />
            </label>
            <label class="grid gap-1 text-xs font-medium text-slate-600">
              排序
        ...
        <MenuPermRulesPanel
          v-if="selectedId > 0 && avatarScopeKeys.length"
          :scope-menu-keys="avatarScopeKeys"
...
const avatarMenus = computed(() =>
  [...menus.value]
    .filter((m) => (m.placement || '').toLowerCase() === 'avatar')
    .sort((a, b) => (a.sort_order !== b.sort_order ? a.sort_order - b.sort_order : a.id - b.id)),
)

const selectedAvatarRow = computed(() =>
  selectedId.value ? avatarMenus.value.find((m) => m.id === selectedId.value) ?? null : null,
)

const avatarScopeKeys = computed(() => {
  const mk = (selectedAvatarRow.value?.menu_key || '').trim()
  return mk ? [mk] : []
})
...
function resetNewForm() {
  ...
  form.icon = ''
  form.path = ''
  form.sort_order = (avatarMenus.value.length ? Math.max(...avatarMenus.value.map((x) => x.sort_order)) : 0) + 10
}
