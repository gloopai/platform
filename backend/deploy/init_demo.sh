#!/usr/bin/env bash
set -euo pipefail
# scaffold/platform-admin：初始化 MySQL（库名默认 platform，与 service-hub / job-worker 配置一致）
...
echo "Ensuring database exists: ${MYSQL_DB} ..."
mysql "${MYSQL_ARGS[@]}" -e "CREATE DATABASE IF NOT EXISTS \`${MYSQL_DB}\` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"
