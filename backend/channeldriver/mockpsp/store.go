...
// SnapshotPayin returns a copy for assertions (optional test helper).
func (s *Store) SnapshotPayin(merchantOrderNo string) (sys string, st channeldriver.PayinOrderStatus, ok bool) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	r, ok := s.payin[merchantOrderNo]
	if !ok {
		return "", 0, false
	}
	return r.sysOrderNo, r.status, true
}
